...
// format to parse the time string on end parameter to return
...
func localizedStringFor(valueType DateAgoValues, value int) string {
	...
	case YearsAgo:
		if value >= 2 {
			return fmt.Sprintf("%d years%s", value, ago)
		} else {
			return "Last year"
		}
	case MonthsAgo:
		if value >= 2 {
			return fmt.Sprintf("%d months%s", value, ago)
		} else {
			return "Last month"
		}
	case WeeksAgo:
		if value >= 2 {
			return fmt.Sprintf("%d weeks%s", value, ago)
		} else {
			return "Last week"
		}
	case DaysAgo:
		if value >= 2 {
			return fmt.Sprintf("%d days%s", value, ago)
		} else {
			return "Yesterday"
		}
	case HoursAgo:
		if value >= 2 {
			return fmt.Sprintf("%d hours%s", value, ago)
		} else {
			return fmt.Sprintf("An hour%s", ago)
		}
	case MinutesAgo:
		if value >= 2 {
			return fmt.Sprintf("%d minutes%s", value, ago)
		} else {
			return fmt.Sprintf("A minute%s", ago)
		}
	case SecondsAgo:
		if value >= 2 {
			return fmt.Sprintf("%d seconds%s", value, ago)
		} else {
			return "Just now"
		}
	}
